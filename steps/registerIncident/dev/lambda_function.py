...
def lambda_handler(event, context):
    ...
    incident_date = datetime.strptime(
        event['incident_date'], "%Y-%m-%d").strftime('%Y-%m-%dT%H:%M:%SZ')
    student_id = event['student_id']

    return create_incident(new_id, incident_date, student_id)


@xray_recorder.capture('put_item')
def create_incident(iid, incident_date, student_id):
